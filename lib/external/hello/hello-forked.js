 ...
 * 
 * Notes: forked from original because the script was adding a trailing slash on my
 * 		  Instagram refresh url. So I commented out all of the calls to:
 * 		  utils.url(options.redirect_uri).href.
 */
	...
	init: function(services, options) {
		...
		if (options) {
			...
			if ('redirect_uri' in options) {
				this.settings.redirect_uri = options.redirect_uri; //utils.url(options.redirect_uri).href;
			}
		}
	...
	login: function() {
		...
		var callbackId = utils.globalEvent(function(str) {
			...
			if (!obj.error) {
				...
				alert('226: ' + JSON.stringify(obj));
				promise.reject(obj);
			}
		});

		var redirectUri = opts.redirect_uri;//utils.url(opts.redirect_uri).href;
	...
	responseHandler: function(window, parent) {
		...
		if (p && 'state' in p) {
			...
			// &state=?
				...
				alert('1424: ' + JSON.stringify(p.error));
