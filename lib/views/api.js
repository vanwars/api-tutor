    ...
    function ($, _, Handlebars, RecordList, hello, CodeMirror) {
        ...
        var APIView = RecordList.extend({
            ...
            checkCredentials: function () {
            	if(!hello.getAuthResponse('instagram')) {
  					this.globals.appRouter.navigate('#/login');
  					return false;
  				} else {
  					//set the access token (so it will be transparent to the learner):
  					this.opts.query_params['access_token'] = hello.utils.store('instagram').access_token;
  					return true;
  				}
            },
  			...
  			search: function (e) {
  				...
  				if(this.collection.query_params["access_token"]) {
  					var session = hello.utils.store('instagram');
  					session.access_token = this.collection.query_params["access_token"];	
  					hello.utils.store('instagram', session);
            ...
            newPage: function (e) {
  			...
  			renderHarness: function () {
                ...
                _.extend(data, { tabs: _.where(this.globals.pages, {type: "instagram" || "soundcloud"}) })
